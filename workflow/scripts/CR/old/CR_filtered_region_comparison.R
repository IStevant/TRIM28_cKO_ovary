...
conditions <- gsub("_R.*", "\\1", colnames(norm_data))
...
plot.pca <- function(matrix, colours, PCs = c("PC1", "PC2")) {
  ...
  col <- as.vector(col)
  conditions <- gsub("_R.*", "\\1", colnames(matrix))
  replicates <- stringr::str_to_sentence(sub(".*_(R[0-9]+)", "\\1", colnames(matrix)))
  ...
  plots <- apply(
    ...
    function(combination) {
      ...
      plot <- ggplot(data, aes(x = PC_x, y = PC_y, fill = cond, color = cond)) +
        ...
        ggtitle("PCA on enriched region coverage")+
        theme_bw() +
...
plot_venn <- function(counts, colours) {
  ...
  conditions <- unique(gsub("_R.*", "\\1", colnames(counts)))
  ...
  print("Overlap region per condition")
  ...
  # Get the total number of enriched region per condition
  ...
  venn_plot$vp$width <- unit(0.6, "npc")
  venn_plot$vp$height <- unit(0.6, "npc")

  title <- ggdraw() + draw_label("Overlap of the enriched regions\nbetween conditions", fontface='bold', size=12)
...
plot_anno <- function(counts, colours) {
  ...
  plot <- ggplot(data, aes(fill = factor(Feature, levels=c("Intergenic", "Promoter", "3' UTR", "Exon", "Intron", "5' UTR")), x = factor(Conditions, level = unique(conditions)), y = Frequency)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = labels, color = factor(Feature, levels=c("Intergenic", "Promoter", "3' UTR", "Exon", "Intron", "5' UTR"))), size = 5, position = position_stack(vjust = 0.5)) +
...
plot_repeat_pct <- function(counts){
  repeats <- rtracklayer::import(repeat_masker)
  conditions <- unique(gsub("_R.*", "\\1", colnames(counts)))
  ...
  anno <- lapply(1:length(anno), function(x) {
    stat <- anno[[x]]
    stat$Conditions <- rep(conditions[x], nrow(stat))
    stat$labels <- paste0(round(stat$Percentage, digit = 2), "%")
    return(stat)
  })
  ...
  plot <- ggplot(data, aes(fill = Feature, x = factor(Conditions, level = unique(conditions)), y = Percentage)) +
    geom_bar(stat = "identity") +
...
repeat_plot <- plot_repeat_pct(counts)

# Combine the two plots
figure <- plot_grid(
  plotlist = list(corr_plot, pca_plot[[1]], venn_plot, anno_plot, repeat_plot),
