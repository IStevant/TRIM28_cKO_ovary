...
log_message("Starting DAR genomic annotation overlap script")
...
if (!"cluster" %in% colnames(DAR_peaks)) {
  log_error("Column 'cluster' not found in DAR table")
}
...
log_message("Computing overlap percentages per DAR cluster")

clusters <- sort(
  unique(DAR_peaks$cluster)
)
...
log_message("Drawing annotation overlap heatmap")
