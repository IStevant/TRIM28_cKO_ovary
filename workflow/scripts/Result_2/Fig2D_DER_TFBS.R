...
filtered_DERs <- read.csv(
  file = snakemake@input[["sig_DERs"]],
  header = TRUE,
  row.names = 1,
  check.names = FALSE
)

if (nrow(filtered_DERs) == 0) {
  log_error("No significant differential regions found")
}

if (!"x" %in% colnames(filtered_DERs)) {
  log_error("Column 'x' containing the cluster/bin assignment was not found")
}

log_message("Number of differential regions: ", nrow(filtered_DERs))
...
get_enriched_TFs <- function(
  ...
  bins <- factor(DERs$x[keep_regions])
